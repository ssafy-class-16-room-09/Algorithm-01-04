...
1. 아래 형식대로 주차 섹션(`## week-NN`)과 표에 문제를 추가한다.
...
- 주차 제목에 `(완료)` 를 붙이면 그 주차는 파일 생성을 건너뛴다 (워크플로 도입 전에 끝난 주차 기록용).
...
2. **파일명**은 `Boj`로 시작하게 짓는다 (예: `Boj1225`) — 표준 입출력용 `main` 스켈레톤이 생성된다.
...
- 로컬(실행 버튼): 저장소 루트에서 아래 명령 실행 → 케이스별 ✅/❌ 와 기대/실제 출력이 표시된다.
  ```
  python tools/judge.py Boj1225
  ```
